...
def show_result(image, save_path, point=None, bbox=None):
    if point is not None and bbox is not None: 
        ...
        # plt.savefig(save_path, bbox_inches='tight', pad_inches=0.0, dpi=200)
        # plt.close()
        media.write_image(save_path.with_suffix(".png"), image, fmt="png")
...
def smooth_GPU(mask, scale = 3):
    # mask = mask.to("cuda").to(torch.float32)
    # threshold = (2*scale+1)**2 // 2
    # h, w = mask.shape
    # kernel = torch.ones(1, 1, 2*scale+1, 2*scale+1).cuda()
    # conv_output = F.conv2d(mask.view(1, 1, h, w), kernel, padding=scale)
    # mask_smoothed = (conv_output > threshold).to(torch.uint8)
    mask = mask.to(torch.float32)
    threshold = (2*scale+1)**2 // 2
    h, w = mask.shape[:2]
    kernel = torch.ones(1, 1, 2*scale+1, 2*scale+1)
    conv_output = F.conv2d(mask.view(1, 1, h, w), kernel, padding=scale)
    mask_smoothed = (conv_output > threshold).to(torch.uint8).view(h, w).numpy()
    
    mask = mask.to(torch.uint8).numpy()
    for i in range(h):
        for j in range(w):
            if i-scale>0 and i+scale+1<h and j-scale>0 and j+scale+1<w:
                continue
            else:
                square = mask[max(0, i-scale) : min(i+scale+1, h-1),
                              max(0, j-scale) : min(j+scale+1, w-1)]
